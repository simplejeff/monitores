...
char szTitleText[ ] = "Monitor ES 0.4";
...
void MediaRunning()
{
        ...
        if(hwndSP != NULL)
         {
           ShowWindow(hwndSP, SW_RESTORE);
           SetFocus(hwndSP);
           SetActiveWindow(hwndSP);
           SendMessage(hwndSP, WM_KEYDOWN, VK_SPACE, 0);
           SendMessage(hwndSP, WM_KEYUP, VK_SPACE, 0);
           ShowWindow(hwndSP, SW_HIDE);
         }
        ...
        if (hwndBS != NULL) {
            ShowWindow(hwndBS, SW_RESTORE);
            SetFocus(hwndBS);
            SetActiveWindow(hwndBS);
            SendMessage(hwndBS, WM_KEYDOWN, VK_SPACE, 0);
            SendMessage(hwndBS, WM_KEYUP, VK_SPACE, 0);
            ShowWindow(hwndBS, SW_SHOWNOACTIVATE);           
        }
        ...
        if (hwndWMPC != NULL) {
         ...
         if(FOOBAR_RUN == 0)                
            {
               controlFooBar(false); // Pause/play 
               FOOBAR_RUN = 1;
            }  
            else    
            {
               controlFooBar(true); // Pause/play                                              
               FOOBAR_RUN = 0;
            } 
}
...
void ControlDigsby()
{
    HWND hwndDBY = FindWindow("wxWindowClassNR",NULL);
    HWND hwndEDITCTRL = FindWindowEx(hwndDBY, NULL, TEXT("wxWindowClass"), NULL);
        if (hwndDBY != NULL) {
            PostMessage(hwndEDITCTRL, WM_COMMAND, 0x01008348, 0x00020384); // Pause/play
        } 
}
