...
void controlFooBar(boolean flag) {
    ...
    if ( Process32First( snapshot, &entry ) == TRUE ) {
    	...
    	while ( Process32Next( snapshot, &entry ) == TRUE ) {
    		if ( cmpistr( entry.szExeFile, "foobar2000.exe" ) == 0 ) { 
                ...
                if(flag)
 			       iReturn = (int) ShellExecute (NULL, "open", command, "/play", NULL, SW_HIDE);
 			    else 
                   iReturn = (int) ShellExecute (NULL, "open", command, "/pause", NULL, SW_HIDE);   
